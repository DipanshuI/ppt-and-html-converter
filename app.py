...
def render_pptx_slides_to_png(pptx_file_path, output_slides_dir):
    ...
    if libreoffice_cmd or not is_windows:
        ...
        res = subprocess.run(
            [cmd_name, "--headless", "--convert-to", "pdf", pptx_file_path, "--outdir", pdf_out_dir],
            capture_output=True,
            text=True
        )

        if res.returncode != 0:
            log_error(f"LibreOffice PDF conversion failed: {res.stderr}")
            raise RuntimeError(f"LibreOffice conversion error: {res.stderr}")

        pdf_files = [f for f in os.listdir(pdf_out_dir) if f.endswith(".pdf")]
        if not pdf_files:
            raise RuntimeError("LibreOffice did not produce a PDF output.")

        pdf_path = os.path.join(pdf_out_dir, pdf_files[0])

        # 2. Render PDF pages into 1080p high-res PNG images using pdf2image
        try:
            ...
            log_error(f"pdf2image conversion failed: {e}")
            raise RuntimeError(f"Failed to render PDF to PNG images: {e}")
